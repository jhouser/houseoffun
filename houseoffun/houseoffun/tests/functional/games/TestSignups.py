...
from houseoffun.houseoffun.tests.util import CssMappings as cm
...
class SignupsTest(BaseStaticLiveServerTestCase):
    ...
    def test_game_signup(self):
        ...
        self.selenium.find_element_by_id(cm.SIGNUP_GAME_LINK_ID).click()
        self.wait.until(ec.url_changes('%s%s' % (self.live_server_url, '/games/')))
        self.selenium.find_element_by_class_name(cm.SIGNUP_BUTTON_CLASS).click()
        self.wait.until(ec.url_contains('%s%s' % (self.live_server_url, '/games/view/')))
        self.selenium.find_element_by_class_name(cm.SIGNUP_STATUS_TEXT_CLASS)
        assert 'Registered' in self.selenium.page_source

    def test_game_withdraw(self):
        self.authenticate()
        self.selenium.get('%s%s' % (self.live_server_url, '/games/'))
        self.selenium.find_element_by_id(cm.WITHDRAW_GAME_LINK_ID).click()
        self.wait.until(ec.url_changes('%s%s' % (self.live_server_url, '/games/')))
        self.selenium.find_element_by_class_name(cm.WITHDRAW_BUTTON_CLASS).click()
        self.wait.until(ec.url_contains('%s%s' % (self.live_server_url, '/games/view/')))
        self.selenium.find_element_by_class_name(cm.SIGNUP_STATUS_TEXT_CLASS)
    ...
    def test_game_accept(self):
        self.authenticate()
        self.selenium.get('%s%s' % (self.live_server_url, '/games/'))
        self.selenium.find_element_by_id(cm.PROCESS_SIGNUPS_GAME_LINK_ID).click()
        self.wait.until(ec.url_changes('%s%s' % (self.live_server_url, '/games/')))
        self.selenium.find_element_by_class_name(cm.ACCEPT_BUTTON_CLASS).click()
        self.wait.until(ec.url_contains('%s%s' % (self.live_server_url, '/games/view/')))
        self.selenium.find_element_by_class_name(cm.SIGNUP_STATUS_TEXT_CLASS)
    ...
    def test_game_reject(self):
        ...
        self.selenium.find_element_by_id(cm.PROCESS_SIGNUPS_GAME_LINK_ID).click()
        self.wait.until(ec.url_changes('%s%s' % (self.live_server_url, '/games/')))
        self.selenium.find_element_by_class_name(cm.REJECT_BUTTON_CLASS).click()
        self.wait.until(ec.url_contains('%s%s' % (self.live_server_url, '/games/view/')))
        self.selenium.find_element_by_class_name(cm.SIGNUP_STATUS_TEXT_CLASS)
